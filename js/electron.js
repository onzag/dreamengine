...
const createWindow = () => {
    ...
    win.webContents.session.setCertificateVerifyProc((request, callback) => {
        ...
        if (config.allowDiffusionSelfSigned) {
            ...
            try {
                const hostUrl = new URL(diffusionHost);
                if (request.hostname === hostUrl.hostname) {
                    callback(0); // trust self-signed cert for diffusion server
                    return;
                }
            } catch {
                // invalid or empty host config, fall through to default
...
ipcMain.handle('startDiffusionProcess', async () => {
...
ipcMain.handle('stopDiffusionProcess', async () => {
