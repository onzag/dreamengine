...
const stdapp = await importScript("@std", "standard-app");
...
engine.exports = {
    ...
    initialize(DE) {
        ...
        const Dema = DE.utils.newCharacter(fss.setup(DE, {
            metadata: stdapp.buildCharacterMetadata({
                assets: {
                    neutral: "@assets/characters/dema-basic/profile",
                },
                sounds: {},
            }),
            name: "Dema",
            ...
            correctiveness: {
                ...
                generalFacts: [
                    ...
                    "Robots are a good source of companionship and assistance",
                    "Robots are not capable of romantic or sexual attraction",
                ],
                questions: [],
            },
            adventurousness: 0.1,
            combat: [
                (info) => `${info.char.name} will use his robotic strength and a series of sharp blades that come off its arm to defend itself from ${info.other.name}`,
            ],
            curiosity: 0.8,
            worldKnowledge: {
                talkingAnimals: false,
                magic: false,
                advancedTechnology: true,
