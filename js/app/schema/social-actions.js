...
const SOCIAL_ACTIONS = [
    {
        ...
        instruction(user, char, currentConversationParticipants) {
...
- ${char.name} is trying to coerce everyone in the conversation to go to [NEW_LOCATION]`;
        ...
        onceEstablishedYesNoQuestion(user, char, currentConversationParticipants, data) {
            ...
            if (data.coercion) {
                if (isEveryone && data.currentTarget === user.name) {
                ...
                } else if (user.name === data.currentTarget) {
                    return `${char.name} is trying to coerce you and ${data.allTargets.filter(t => t !== user.name)} to go to ${data.newLocationSlot}, what would you do?`;
                } else {
            ...
            if (user.name === data.currentTarget) {
                return `${char.name} wants to move away from the group and go with ${data.allTargets.map((t) => t !== user.name ? "you" : t)}, do you accept?`;
            } else {
        ...
        onceEstablishedYesNoQuestionConfirm(user, char, currentConversationParticipants, data) {
            // Yes even the user would have to answer this question
            if (data.negatingTargets.length === 0) {
                return null;
            }
            if (data.negatingTargets.length >= 1 && char.name === user.name) {
        ...
        onceEstablishedYesNoQuestionConfirmUserOptions(user, char, currentConversationParticipants, data) {
            if (data.coercion) {
                return {
                    "Fight back": "*" + user.name + " fights back*\n\n",
                    "Allow it": "*" + user.name + " allows " + (user.gender === "ambiguous" ? "themself" : (user.gender === "male" ? "himself" : "herself")) + " to be taken*\n\n",
                }
            }
            return {
                "Yes": "*" + user.name + " agrees to go*\n\n",
                "No": "*" + user.name + " declines to go*\n\n",
            }
        },
        onceEstablishedYesNoQuestionEvenAskerNegatedConfirm(user, char, currentConversationParticipants, data) {
            // Yes even the user would have to answer this question
            if (data.newNegatingTargets.length >= 1 && char.name === user.name) {
        ...
        async parseResponseFromInstruction(user, char, currentConversationParticipants, response, DE) {
        ...
        parseResponseFromConfirmation(user, char, currentConversationParticipants, response, DE) {
            // TODO
            return {
        ...
        execute(user, char, currentConversationParticipants, newConversationParticipants, DE) {
...
`;
        ...
        parseResponseFromInstruction(user, char, currentConversationParticipants, response, DE) {
            // TODO
            return {
                matched: false,
                coercion: false,
        ...
        async execute(user, char, currentConversationParticipants, newConversationParticipants, DE) {
            // TODO update the game state
