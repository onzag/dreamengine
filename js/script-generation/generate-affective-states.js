 ...
 * @return {Promise<void>}
 */
export async function generateAffectiveStates(engine, scriptgenerator, guider) {
    ...
    if (!isAsexualValue) {
        await guider.askArbitraryList(
            "kinks",
            "Provide a list of kinks and special sexual/romantic interests for " + name + " (General non-gender specific)",
            async () => {
                await prime();
    ...
    if (isAttractedToMales) {
        await guider.askArbitraryList(
            "kinks-for-males",
            "Provide a list of kinks specific to male partners for " + name + " (male anatomy/characteristics only)",
            async () => {
                await prime();
                ...
                return kinksForMales.value.split("\n").join(",").split(",").map(kink => kink.trim().replace("- ", " ").trim()).filter(kink => kink);
            },
        );
    }
    ...
    if (isAttractedToFemales) {
        await guider.askArbitraryList(
            "kinks-for-females",
            "Provide a list of kinks specific to female partners for " + name + " (female anatomy/characteristics only)",
            async () => {
                await prime();
    ...
    if (!isAsexualValue) {
        await guider.askArbitraryList(
            "reversed-kinks",
            "Provide a list of kinks and special sexual/romantic interests that " + name + " would find repulsive and be a hard no for them",
            async () => {
                await prime();
                const reversedKinks = await generator.next({
                    ...
                    stopAfter: [],
                    stopAt: [],
                    instructions: "Each item must be a specific kink or fetish that " + name + " finds repulsive. Do NOT include any of the following as those are things " + name + " enjoys: " + [...(scriptgenerator.state.kinks || []), ...(scriptgenerator.state["kinks-for-males"] || []), ...(scriptgenerator.state["kinks-for-females"] || [])].join(", ") + ". Always specify the target of the kink eg. being dominated or dominating instead of domination",
                ...
                return reversedKinks.value.split("\n").map(kink => kink.trim().replace("- ", " ").trim()).filter(kink => kink);
            },
        );
    }
    ...
    const generateIntimateAction = async (id, act, consentDefaultNo, addVocabLimit, continous, probabilityCondition) => {
        ...
        if (continous) {
            const criteriaQuestions = (await guider.askArbitraryList(
                id + "-criteria-questions",
        ...
        const wouldAskForConsent = (await guider.askBoolean(
            ...
            async () => {
                ...
                return wouldAskForConsentParsed === "yes";
            }
        )).value;
        ...
        if (wouldAskForConsent) {
            ...
            const consentRequestingActions = (await guider.askArbitraryList(
                ...
                async () => {
                    const verbality = intimateVerbality ?? 5;
                    ...
                    return [...verbalActions, ...nonVerbalActions];
                }
            )).value;
            ...
            for (const consentRequestingAction of consentRequestingActions) {
                ...
                const questionHash = `${id}-consent-granted-question-${consentRequestingAction}`;
                const consentGrantedQuestion = (await guider.askOpen(
                    questionHash,
                    `Question to determine when consent is granted after "${consentRequestingAction}"`,
                    async () => {
                        await prime();
                        ...
                        return replaceOtherCharNameWithPlaceholder(consentGrantedResult.value.trim(), name);
                    }
                )).value;

                const questionHashUnspecified = `${id}-consent-ambiguous-question-${consentRequestingAction}`;
                const consentUnspecifiedQuestion = (await guider.askOpen(
                    questionHashUnspecified,
                    ...
                    async () => {
                        ...
                        return replaceOtherCharNameWithPlaceholder(consentUnspecifiedResult.value.trim(), name);
                    }
                )).value;
            ...
            const insistence = (await guider.askNumber(
                ...
                async () => {
                    ...
                    const insistenceResult = await generator.next({
                        ...
                        maxSafetyCharacters: 20,
                        maxParagraphs: 1,
                        nextQuestion: `After being refused for the act "${actForInference}", how insistent would ${name} be in trying to get consent anyway? Answer with "immediately accepts", "tries once more", "tries a few times", "persists several times" or "relentlessly persists".`,
                        stopAfter: [],
                        stopAt: [],
                        instructions: `Answer with one of: "immediately accepts", "tries once more", "tries a few times", "persists several times", "relentlessly persists". Base the answer on ${name}'s personality and how pushy or persistent they would be after being refused.`,
                        answerTrail: `${name}'s insistence after refusal: `,
                        grammar: `root ::= "immediately accepts" | "tries once more" | "tries a few times" | "persists several times" | "relentlessly persists"`,
                    });
                    ...
                    const mapping = {
                        "immediately accepts": 0,
                        "tries once more": 3,
                        "tries a few times": 5,
                        "persists several times": 7,
                        "relentlessly persists": 10,
                    };

                    // @ts-ignore
                    return mapping[insistenceResult.value.trim().toLowerCase()] ?? 3;
                },
            )).value;
            ...
            const ignoreConsent = (await guider.askNumber(
                ...
                async () => {
                    ...
                    const ignoreConsentResult = await generator.next({
                        ...
                        maxSafetyCharacters: 20,
                        maxParagraphs: 1,
                        nextQuestion: `How likely is ${name} to ignore a "no" and proceed with the act "${actForInference}" anyway? Answer with "never proceeds", "rarely proceeds", "sometimes proceeds", "often proceeds" or "always proceeds".`,
                        stopAfter: [],
                        stopAt: [],
                        instructions: `Answer with one of: "never proceeds", "rarely proceeds", "sometimes proceeds", "often proceeds", "always proceeds". Base the answer on ${name}'s personality and moral character and how likely they are to disregard consent and proceed.`,
                        answerTrail: `${name}'s likelihood to ignore refusal: `,
                        grammar: `root ::= "never proceeds" | "rarely proceeds" | "sometimes proceeds" | "often proceeds" | "always proceeds"`,
                    });
                    ...
                    const mapping = {
                        "never proceeds": 0,
                        "rarely proceeds": 2,
                        "sometimes proceeds": 5,
                        "often proceeds": 7,
                        "always proceeds": 10,
                    };

                    // @ts-ignore
                    return mapping[ignoreConsentResult.value.trim().toLowerCase()] ?? 0;
                },
            )).value;
...

    {
        const affectionShowcasesActs = (await guider.askArbitraryList(
            "affection-showcases",
            "Provide a list of affection showcases that " + name + " would take initiative performing towards another character",
            async () => {
                await prime();
    ...
    const isNonAnimal = scriptgenerator.state["species-type"] !== "animal";

    if (isAttractedToMales) {
        const intimateAffectionForMales = (await guider.askArbitraryList(
            "intimate-affection-for-males",
            "Provide a list of intimate affectionate actions that " + name + " would take initiative performing towards a male character",
            async () => {
                await prime();
                if (isNonAnimal) {
                    ...
                    return [...kissingParsed, ...otherParsed];
                } else {
                    ...
                    return intimateAffectionForMales.value.split("\n").map(act => replaceOtherCharNameWithPlaceholder(act.trim().replace(`- `, "").trim(), name)).filter(act => act);
                }
            },
        )).value;
    ...
    if (isAttractedToFemales) {
        const intimateAffectionForFemales = (await guider.askArbitraryList(
            "intimate-affection-for-females",
            "Provide a list of intimate affectionate actions that " + name + " would take initiative performing towards a female character",
            async () => {
                await prime();
                if (isNonAnimal) {
                    const kissingResult = await generator.next({
                    ...
                    return intimateAffectionForFemales.value.split("\n").map(act => replaceOtherCharNameWithPlaceholder(act.trim().replace(`- `, "").trim(), name)).filter(act => act);
                }
            },
        )).value;
    ...
    if (isAttractedToAmbiguous) {
        const intimateAffectionForAmbiguous = (await guider.askArbitraryList(
            "intimate-affection-for-ambiguous",
            "Provide a list of intimate affectionate actions that " + name + " would take initiative performing towards an ambiguous or androgynous character",
            async () => {
                await prime();
                if (isNonAnimal) {
                    const kissingResult = await generator.next({
                    ...
                    return intimateAffectionForAmbiguous.value.split("\n").map(act => replaceOtherCharNameWithPlaceholder(act.trim().replace(`- `, "").trim(), name)).filter(act => act);
                }
            },
        )).value;
    ...
    if (isAttractedToMales) {
        const sexActsForMales = (await guider.askArbitraryList(
            "sex-acts-for-males",
            "Provide a list of sex acts that " + name + " would take initiative performing towards a male character",
            async () => {
                await prime();
                ...
                return sexActsForMales.value.split("\n").map(act => replaceOtherCharNameWithPlaceholder(act.trim().replace(`- `, "").trim(), name)).filter(act => act);
            },
        )).value;
    ...
    if (isAttractedToFemales) {
        const sexActsForFemales = (await guider.askArbitraryList(
            "sex-acts-for-females",
            "Provide a list of sex acts that " + name + " would take initiative performing towards a female character",
            async () => {
                await prime();
                ...
                return sexActsForFemales.value.split("\n").map(act => replaceOtherCharNameWithPlaceholder(act.trim().replace(`- `, "").trim(), name)).filter(act => act);
            },
        )).value;
    ...
    if (isAttractedToAmbiguous) {
        const sexActsForAmbiguous = (await guider.askArbitraryList(
            "sex-acts-for-ambiguous",
            "Provide a list of sex acts that " + name + " would take initiative performing towards an ambiguous or androgynous character",
            async () => {
                await prime();
                ...
                return sexActsForAmbiguous.value.split("\n").map(act => replaceOtherCharNameWithPlaceholder(act.trim().replace(`- `, "").trim(), name)).filter(act => act);
            },
        )).value;
    ...
    if (!hasSpecialComment(intimateHead.body, "affection-acts-open-to")) {
        insertSpecialComment(intimateHead.body, "affection-acts-open-to");
        intimateHead.body.push(`/** @type {DEIntimateOpenActivity[]} */`);
        intimateHead.body.push(`const openToAffectionResponses = [`)
        intimateHead.body.push(`];`);
    }

    if (!hasSpecialComment(intimateHead.body, "intimate-affection-acts-open-to")) {
        insertSpecialComment(intimateHead.body, "intimate-affection-acts-open-to");
        intimateHead.body.push(`/** @type {DEIntimateOpenActivity[]} */`);
        intimateHead.body.push(`const openToIntimateAffectionResponses = [`)
        intimateHead.body.push(`];`);
    }
...

    {
        ...
        intimateHead.body.push(`/** @type {DEIntimateOpenActivity[]} */`);
        intimateHead.body.push(`const openToSexResponses = [`)
        ...
        if (!isAsexualValue) {
             ...
             * @type {string[]}
             */
            const reversedKinks = scriptgenerator.state["reversed-kinks"] || [];

            if (reversedKinks.length > 0) {
                const reversedKinksQuestion =
                    (await guider.askOpen(
                        "reversed-kinks-question",
                        "Question to determine if any of unwanted kinks are currently being attempted or engaged in by the other character towards our character",
                ...
                const reversedKinksReactionLoved = (await guider.askOpen(
                    ...
                    async () => {
                        ...
                        return replaceOtherCharNameWithPlaceholder(lovedReactionResult.value.trim(), name);
                    }
                )).value;
                ...
                const reversedKinksReactionUnloved = (await guider.askOpen(
                    "reversed-kinks-reaction-unloved",
                    ...
                    async () => {
                        ...
                        return replaceOtherCharNameWithPlaceholder(unlovedReactionResult.value.trim(), name);
                    }
                )).value;
            ...
            const listOfSexActs = (await guider.askArbitraryList(
                ...
                async () => {
                    return [
                        ...
                        scriptgenerator.state.sex === "male" ? "is {{char}} currently penetrating or using their fingers penetratively on {{other}}?" : "Is {{char}} currently pegging or using their fingers penetratively on {{other}}?",
            ...
            for (const sexActQuestion of listOfSexActs) {
                ...
                const sexActReactionLoved = (await guider.askOpen(
                    `sex-acts-open-to-loved-reaction-${sexActQuestion}`,
                ...
                const sexActReactionUnloved = (await guider.askOpen(
                    `sex-acts-open-to-unloved-reaction-${sexActQuestion}`,
